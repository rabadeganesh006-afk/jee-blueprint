...
import {
  BookOpen, Bot, Check, CheckCircle2, ChevronDown, Circle, Clock, FileText, Home, Mail,
  MessageCircle, Moon, PenLine, Pi, Plus, Save, Search, Send, ShieldCheck, Star, Sun,
  Target, Trash2, Trophy, UserRound, X, RotateCcw
} from 'lucide-react';
...
function createDemoData(planner) {
  ...
  return {
    ...
    profile: {
      ...
      weakAreas: 'Rotational Motion, Ionic Equilibrium, Definite Integration',
...
export default function DemoApp() {
  ...
  const searchResults = query.trim() ? [['dashboard', 'Dashboard'], ['pyq', 'PYQ Practice'], ['material', 'Study Material'], ['tests', 'Test Series'], ['ai', 'AI Tutor'], ['contact', 'Contact Us'], ['profile', 'Profile']].map(([page, label]) => ({ page, label, type: 'Page' })) : [];
  ...
  return (
    <div className={`app ${data.theme === 'dark' ? 'dark' : 'light'}`}>
      ...
      <main className="main">
        ...
        {data.active === 'privacy' && <PrivacyPage />}
...
function PyqPage({ data, stream, pyqSets, selectedPyq, setSelectedPyq, markPyqSolved, query }) {
  ...
  const subjectCounts = subjectOrder.map((subject) => ({ subject, count: pyqSets.filter((set) => set.subject === subject).length, visibleCount: sets.filter((set) => set.subject === subject).length, solved: pyqSets.filter((set) => set.subject === subject).reduce((sum, set) => sum + Number(data.pyqSolved?.[set.id] || 0), 0) }));
...
function AiPage() { return <section className="page"><div className="pageHead"><h1>AI Tutor</h1><p>This feature is temporarily disabled while we improve reliability.</p></div><div className="card aiTutorPanel comingSoonPanel"><div className="aiTutorTop"><div className="aiAvatar"><Bot size={30} /></div><div><span className="badge">Coming Soon</span><h2>Study Blueprint AI Tutor is being upgraded</h2><p>AI Tutor is currently turned off to avoid wrong responses, quota errors, and unstable experience for students.</p></div></div><div className="comingSoonGrid"><div><b>Current status</b><span>Disabled for now</span></div><div><b>Reason</b><span>Reliability and quota issues</span></div><div><b>Next plan</b><span>We will bring it back after proper testing</span></div></div><div className="safeBox">For now, continue using Topic Tracker, PYQ Practice, Study Material, Test Series, and Study Timer.</div></div></section>; }
function ContactPage({ data }) { const [form, setForm] = useState({ name: data.profile?.fullName || '', email: data.profile?.email || '', reason: 'General enquiry', message: '' }); const [status, setStatus] = useState(''); function submitContact(event) { event.preventDefault(); setStatus('Demo message saved. In real mode this will be sent to support.'); } return <section className="page contactPage"><div className="pageHead contactHeroHead"><div><h1>Contact Us</h1><p>For questions, feedback, coaching customization, bug reports, or collaboration requests.</p></div><div className="contactHeroBadge"><MessageCircle size={20} /> Student support ready</div></div><div className="contactLayout"><form className="card contactForm" onSubmit={submitContact}><h3>Send a message</h3><p className="mutedText">Send your question, feedback, bug report, or collaboration request. We will review it and reply as soon as possible.</p><div className="formGrid2"><label>Name<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Your name" /></label><label>Email<input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="your@email.com" /></label></div><label>Reason<select value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })}><option>General enquiry</option><option>Coaching/class custom website</option><option>Bug report</option><option>Feature request</option><option>Content partnership</option></select></label><label>Message<textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Write your message here..." rows={6} /></label><button className="primary" type="submit"><Send size={17} /> Submit message</button>{status && <div className="contactStatusBox">{status}</div>}</form><aside className="contactSideStack"><div className="card contactInfoCard"><Mail size={24} /><h3>We are here to help</h3><p>For support, feedback, partnership, or coaching-class customization, send a clear message with your email address.</p></div><div className="card contactInfoCard safe"><ShieldCheck size={24} /><h3>Safe contact only</h3><p>Please do not share passwords, OTPs, API keys, payment details, or private student information in this form.</p></div><div className="card"><h3>Recent sent messages</h3><p className="empty">No contact messages saved in this browser yet.</p></div></aside></div></section>; }
function PrivacyPage() { return <section className="page legalPage"><div className="legalHeader"><span className="landingBadge">Privacy First</span><h1>Privacy Policy</h1><p>Last updated: 23 June 2026</p></div><div className="legalCardStack"><article className="card legalCard"><h3>What we collect</h3><p>Study Blueprint asks for only the basic details needed to personalize your dashboard: name, email, class, exam target, target date, preferred language, study goal, topic progress, PYQ progress, study timer activity and support messages.</p></article><article className="card legalCard"><h3>What we never ask for</h3><p>Please do not share passwords, OTPs, payment card details, home address, private documents or any sensitive personal information inside the app or contact form.</p></article><article className="card legalCard"><h3>How we use data</h3><p>Your details are used to show your study dashboard, save your profile, track your target date, record progress, answer support messages and improve the app experience.</p></article><article className="card legalCard"><h3>Your choices</h3><p>You can update your profile, clear browser data, or request account data removal from the Profile page in real mode.</p></article></div></section>; }
function ProfilePage({ data }) { const fields = [['fullName', 'Full Name'], ['mobile', 'Mobile No'], ['email', 'Email'], ['className', 'Class'], ['board', 'Board / State Board'], ['language', 'Preferred Language'], ['studyGoal', 'Study Goal'], ['dailyStudyTime', 'Daily Study Time'], ['weakAreas', 'Weak Areas']]; return <section className="page profilePageV23"><div className="pageHead"><h1>Profile</h1><p>Manage student details, exam target, profile photo and account settings.</p></div><div className="profileHero profileHeroV23"><div className="profileIdentityBlock"><div className="profilePhotoWrap"><div className="avatar profileAvatarFallback">👨‍🎓</div><label className="photoUploadBtn" title="Add or change profile photo"><PenLine size={15} /></label></div><div className="profileIdentityText"><h2>{data.profile.fullName || 'Student Profile'}</h2><p>{data.profile.email || 'Email not available'}</p><span className="badge">{data.profile.targetExam || 'Target not set'}</span></div></div><div className="profileFutureCard"><div className="futureGraphic" aria-hidden="true"><span className="rocketShape">🚀</span><i></i><i></i><i></i></div><div><h3>Keep building your future</h3><p>Complete your profile to get a cleaner dashboard, better target tracking and personalized study planning.</p></div></div><div className="targetSummary targetSummaryV23"><span>Target Date</span><b>{data.profile.targetDate ? formatDate(data.profile.targetDate) : 'Date not set'}</b><small>Set target date</small></div></div><div className="card profileTargetCard profileTargetV23"><div><h3>Exam Target</h3><p className="muted">Set the target exam name and date here. The dashboard will use this target for tracking.</p></div><div className="targetForm"><label><span>Target Exam Name</span><input value={data.profile.targetExam || ''} readOnly /></label><label><span>Target Exam Date</span><input value={data.profile.targetDate || ''} type="date" readOnly /></label><button className="primary"><Save size={16}/> Save Target</button></div></div><div className="profileGrid profileGridV23"><div className="card wide"><div className="between"><h3>Personal & Academic Details</h3><button><PenLine size={16}/> Edit</button></div><div className="detailsGrid detailsGridV23">{fields.map(([key, label]) => <label key={key}><span>{label}</span><b>{data.profile[key] || '-'}</b></label>)}</div></div><div className="profileSideStack"><div className="card actions actionsV23"><h3>Account Actions</h3><button className="primary"><PenLine size={16}/> Edit Profile</button><button><Save size={16}/> Save Changes</button><button><RotateCcw size={16}/> Reset this browser data</button><button className="danger softDanger"><Trash2 size={16}/> Demo mode only</button></div></div></div></section>; }
