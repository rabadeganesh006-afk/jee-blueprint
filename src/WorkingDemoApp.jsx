...
import {
  BookOpen, Bot, Check, CheckCircle2, ChevronDown, Circle, Clock, Home, Mail, MessageCircle,
  Moon, PenLine, Pi, Plus, RotateCcw, Save, Search, Send, ShieldCheck, Star, Sun, Target,
...
function chapterDifficulty(subject, title = '', topicCount = 0) {
  const text = title.toLowerCase();
  if (topicCount >= 18) return 'High';
  if (subject === 'Mathematics' && /(integration|differential|vector|3d|complex|probability|conic)/.test(text)) return 'High';
  if (subject === 'Physics' && /(rotation|electrostatics|current|magnet|waves|thermo|modern)/.test(text)) return 'High';
  if (subject === 'Chemistry' && /(bonding|equilibrium|organic|coordination|electrochemistry|kinetics)/.test(text)) return 'High';
  return topicCount <= 5 ? 'Easy' : 'Medium';
}
...
function makeDemoData(planner) {
  ...
  pyqSets.slice(0, 10).forEach((set, index) => { pyqSolved[set.id] = 8 + index * 2; });
  return {
    theme: 'light',
    stream: '11th IIT JEE',
    active: 'dashboard',
    ...
    tests: [
      { id: 'test-1', name: 'JEE Main Full Mock 01', score: 142, total: 300, date: '10 Jun 2026' },
      { id: 'test-2', name: 'JEE Main Full Mock 02', score: 168, total: 300, date: '17 Jun 2026' },
      { id: 'test-3', name: 'JEE Advanced Paper 1', score: 78, total: 180, date: '20 Jun 2026' },
    ],
    tasks: [
      { id: 'task-1', title: 'Revise Rotational Motion formula sheet', done: false, date: todayKey() },
      { id: 'task-2', title: 'Solve 30 PYQs from Ionic Equilibrium', done: false, date: todayKey() },
      { id: 'task-3', title: 'Complete Definite Integration mixed practice', done: false, date: todayKey() },
    ],
    activities: [
      { id: 'act-1', text: 'Completed topic: System of Units (Units and Measurements)', date: new Date().toLocaleString() },
      { id: 'act-2', text: 'Added test score: 168/300', date: new Date().toLocaleString() },
      { id: 'act-3', text: 'Solved 5 PYQ in Mathematical Tools', date: new Date().toLocaleString() },
    ],
    profile: {
      fullName: 'Ganesh Rabade', email: 'demo@student.com', mobile: 'Demo mode', city: 'Pune', className: '12', board: 'Demo Board',
      targetExam: 'JEE Advanced 2027', targetDate: '', language: 'English', studyGoal: 'Get into IIT', dailyStudyTime: '5-6 hours',
      weakAreas: 'Rotational Motion, Ionic Equilibrium, Definite Integration', preferredContent: 'Video lectures, Notes, PYQs', avatarDataUrl: '',
    },
  };
}

export default function WorkingDemoApp() {
  const streams = ['11th IIT JEE', '12th IIT JEE', 'Dropper'];
  const initialPlanner = useMemo(() => PLANNER['11th IIT JEE'], []);
  const [data, setData] = useState(() => makeDemoData(initialPlanner));
  ...
  function pushActivity(current, text) { return [{ id: `${Date.now()}`, text, date: new Date().toLocaleString() }, ...(current.activities || [])].slice(0, 30); }
  function setActive(active) { update((current) => ({ ...current, active })); }
  function toggleTopic(topic, chapterTitle) { update((current) => { const nextValue = !current.topicsDone?.[topic.id]; return { ...current, topicsDone: { ...(current.topicsDone || {}), [topic.id]: nextValue }, activities: nextValue ? pushActivity(current, `Completed topic: ${topic.title} (${chapterTitle})`) : current.activities }; }); }
  ...
  function startStudyTimer(goalMinutes) { update((current) => ({ ...current, studyTimer: { ...current.studyTimer, targetMinutes: goalMinutes, elapsedSeconds: 25 * 60, running: false }, activities: pushActivity(current, `Started study timer (${minutesText(goalMinutes)} target)`) })); }
  ...
  function markPyqSolved(setId, count) { const set = pyqSets.find((item) => item.id === setId); update((current) => ({ ...current, pyqSolved: { ...(current.pyqSolved || {}), [setId]: Math.min((current.pyqSolved?.[setId] || 0) + count, set?.questions || 999) }, activities: pushActivity(current, `Solved ${count} PYQ in ${set?.title || 'PYQ set'}`) })); }
  function toggleMaterial(id) { update((current) => ({ ...current, materialRead: { ...(current.materialRead || {}), [id]: !current.materialRead?.[id] }, activities: pushActivity(current, 'Updated study material progress') })); }
  ...
  function saveProfile(nextProfile = profileDraft) { update((current) => ({ ...current, profile: { ...current.profile, ...nextProfile }, activities: pushActivity(current, 'Profile details updated') })); setProfileDraft((current) => ({ ...current, ...nextProfile })); setEditingProfile(false); }
  function resetDemoData() { const fresh = makeDemoData(PLANNER['11th IIT JEE']); setData(fresh); setProfileDraft(fresh.profile); setEditingProfile(false); }

  const nav = [
    { id: 'dashboard', label: 'Dashboard', group: 'LEARN', icon: Home }, { id: 'pyq', label: 'PYQ Practice', group: 'LEARN', icon: Pi },
    { id: 'material', label: 'Study Material', group: 'LEARN', icon: BookOpen }, { id: 'tests', label: 'Test Series', group: 'ASSESS', icon: CheckCircle2 },
    { id: 'ai', label: 'AI Tutor', group: 'AI LEARNING', icon: Bot }, { id: 'contact', label: 'Contact Us', group: 'ACCOUNT', icon: Mail },
    { id: 'profile', label: 'Profile', group: 'ACCOUNT', icon: UserRound }, { id: 'privacy', label: 'Privacy Policy', group: 'LEGAL', icon: ShieldCheck },
  ];
  const searchResults = query.trim() ? nav.map((item) => ({ page: item.id, label: item.label, type: 'Page' })) : [];

  return <div className={`app ${data.theme === 'dark' ? 'dark' : 'light'}`}>
    <aside className="sidebar"><div className="brand brandFull"><img className="brandLogoFull" src="/study-blueprint-logo-new.png" alt="Study Blueprint" /></div>{['LEARN', 'ASSESS', 'AI LEARNING', 'ACCOUNT', 'LEGAL'].map((group) => <div className="navGroup" key={group}><p>{group}</p>{nav.filter((item) => item.group === group).map((item) => { const Icon = item.icon; return <button key={item.id} onClick={() => setActive(item.id)} className={`navBtn ${data.active === item.id ? 'active' : ''}`}><Icon size={18} /> <span>{item.label}</span></button>; })}</div>)}<div className="aiHelp comingSoonMini"><Bot size={42} /><strong>AI Tutor</strong><span>Coming soon. This feature is temporarily disabled while we improve reliability.</span><button onClick={() => setActive('ai')}>View Status</button></div></aside>
    <main className="main"><header className="topbar"><div className="welcome">Welcome back, {displayName} 👋</div><div className={`selectWrap customSelect ${streamMenuOpen ? 'open' : ''}`}><button type="button" className="selectButton" onClick={() => setStreamMenuOpen((v) => !v)}><span>{data.stream}</span><ChevronDown size={16} /></button>{streamMenuOpen && <div className="selectMenu">{streams.map((option) => <button key={option} type="button" className={data.stream === option ? 'active' : ''} onClick={() => { update((current) => ({ ...current, stream: option })); setStreamMenuOpen(false); }}><span>{option}</span>{data.stream === option && <Check size={15} />}</button>)}</div>}</div><div className="searchWrap"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search chapters, topics, PYQs, notes..." /><kbd>Ctrl</kbd><kbd>K</kbd>{searchResults.length > 0 && <div className="searchResults">{searchResults.map((r) => <button key={r.page} onClick={() => setActive(r.page)}><span>{r.label}</span><small>{r.type}</small></button>)}</div>}</div><div className="themeToggle"><button className={data.theme === 'light' ? 'active' : ''} onClick={() => update((current) => ({ ...current, theme: 'light' }))}><Sun size={15} /> Light</button><button className={data.theme === 'dark' ? 'active' : ''} onClick={() => update((current) => ({ ...current, theme: 'dark' }))}><Moon size={15} /> Dark</button></div><button className="planBtn"><Trophy size={16} /> Free Plan</button></header>
      {data.active === 'dashboard' && <Dashboard data={data} planner={planner} progressValue={progressValue} doneCount={doneCount} totalTopics={totalTopics} totalChapters={allChapters.length} todayMinutes={todayMinutes} liveTimerSeconds={data.studyTimer.elapsedSeconds || 0} pyqSolved={pyqSolved} avgMinutes={225} startStudyTimer={startStudyTimer} resetStudyTimer={resetStudyTimer} toggleTopic={toggleTopic} toggleFlagTopic={toggleFlagTopic} addTask={addTask} newTask={newTask} setNewTask={setNewTask} toggleTask={toggleTask} deleteTask={deleteTask} deleteActivity={deleteActivity} clearActivities={clearActivities} setActive={setActive} query={query} />}
      ...
      {data.active === 'ai' && <AiPage />}{data.active === 'contact' && <ContactPage data={data} />}{data.active === 'privacy' && <PrivacyPage />}
      {data.active === 'profile' && <ProfilePage data={data} profileDraft={profileDraft} setProfileDraft={setProfileDraft} editingProfile={editingProfile} setEditingProfile={setEditingProfile} saveProfile={saveProfile} resetDemoData={resetDemoData} />}
    </main>
  </div>;
}

function Dashboard({ data, planner, progressValue, doneCount, totalTopics, totalChapters, todayMinutes, liveTimerSeconds, pyqSolved, avgMinutes, startStudyTimer, resetStudyTimer, toggleTopic, toggleFlagTopic, addTask, newTask, setNewTask, toggleTask, deleteTask, deleteActivity, clearActivities, setActive, query }) {
  ...
  let currentAngle = 0; const ringParts = [];
...
function PyqPage({ data, stream, pyqSets, selectedPyq, setSelectedPyq, markPyqSolved, query }) {
  ...
  const groupedSets = subjectOrder.map((subject) => ({ subject, sets: sets.filter((set) => set.subject === subject) })).filter((group) => group.sets.length > 0);
  return <section className="page"><div className="pageHead"><h1>PYQ Practice</h1><p>{stream} chapter-wise PYQ practice arranged subject-wise in the same order as your lecture planner. Solved count increases only after real practice.</p></div><div className="pyqSubjectStack">{groupedSets.map((group) => <section className="pyqSubjectSection" key={group.subject}><div className="subjectSectionHead"><div><span className="subjectDot" style={{ background: SUBJECT_COLORS[group.subject] }} /><h2>{group.subject}</h2></div><small>{group.sets.length} chapters • planner order</small></div><div className="cards4 pyqChapterGrid">{group.sets.map((set) => { const solved = Number(data.pyqSolved?.[set.id] || 0); return <div className="card pyqChapterCard" key={set.id}><div className="chapterTopLine"><span className="badge">{set.subject}</span><small>Chapter {set.plannerOrder}</small></div><h3>{set.title}</h3><p>{set.topicCount} planner topics linked</p><p>{set.questions} PYQs planned • {set.difficulty}</p><div className="smallProgress"><i style={{ width: `${Math.min(100, percentNumber(solved, set.questions))}%`, background: `linear-gradient(90deg, ${SUBJECT_COLORS[set.subject]}, #60a5fa)` }} /></div><small>{solved} / {set.questions} solved</small><button className="primary" onClick={() => setSelectedPyq(set.id)}>Open set</button></div>; })}</div></section>)}</div>{current && <div className="card mt stickySetCard"><div><span className="badge">{current.subject}</span><h3>{current.title}</h3></div><p>Solved: {data.pyqSolved?.[current.id] || 0} / {current.questions}</p><div className="btnRow"><button onClick={() => markPyqSolved(current.id, 1)}>Mark 1 solved</button><button onClick={() => markPyqSolved(current.id, 5)}>+5 solved</button><button onClick={() => setSelectedPyq(null)}>Close</button></div></div>}</section>;
}
function MaterialPage({ data, toggleMaterial, query }) { const q = query.trim().toLowerCase(); const items = q ? MATERIALS.filter((item) => `${item.title} ${item.subject} ${item.detail}`.toLowerCase().includes(q)) : MATERIALS; return <section className="page"><div className="pageHead"><h1>Study Material</h1><p>Mark materials as read only after you actually complete them.</p></div><div className="cards4">{items.map((m) => <div className="card" key={m.id}><span className="badge">{m.subject}</span><h3>{m.title}</h3><p>{m.detail}</p><button className={data.materialRead?.[m.id] ? 'successBtn' : 'primary'} onClick={() => toggleMaterial(m.id)}>{data.materialRead?.[m.id] ? 'Read ✓' : 'Mark as read'}</button></div>)}</div></section>; }
...
function AiPage() { return <section className="page"><div className="pageHead"><h1>AI Tutor</h1><p>This feature is temporarily disabled while we improve reliability.</p></div><div className="card aiTutorPanel comingSoonPanel"><div className="aiTutorTop"><div className="aiAvatar"><Bot size={30} /></div><div><span className="badge">Coming Soon</span><h2>Study Blueprint AI Tutor is being upgraded</h2><p>AI Tutor is currently turned off to avoid wrong responses, quota errors, and unstable experience for students.</p></div></div><div className="safeBox">For now, continue using Topic Tracker, PYQ Practice, Study Material, Test Series, and Study Timer.</div></div></section>; }
function ContactPage({ data }) { const [form, setForm] = useState({ name: data.profile.fullName, email: data.profile.email, reason: 'General enquiry', message: '' }); const [status, setStatus] = useState(''); return <section className="page contactPage"><div className="pageHead contactHeroHead"><div><h1>Contact Us</h1><p>For questions, feedback, coaching customization, bug reports, or collaboration requests.</p></div><div className="contactHeroBadge"><MessageCircle size={20} /> Student support ready</div></div><div className="contactLayout"><form className="card contactForm" onSubmit={(event) => { event.preventDefault(); setStatus('Demo message saved. In real mode this will be sent to support.'); }}><h3>Send a message</h3><p className="mutedText">Send your question, feedback, bug report, or collaboration request.</p><div className="formGrid2"><label>Name<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label>Email<input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label></div><label>Reason<select value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })}><option>General enquiry</option><option>Coaching/class custom website</option><option>Bug report</option><option>Feature request</option></select></label><label>Message<textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} rows={6} /></label><button className="primary" type="submit"><Send size={17} /> Submit message</button>{status && <div className="contactStatusBox">{status}</div>}</form><aside className="contactSideStack"><div className="card contactInfoCard"><Mail size={24} /><h3>We are here to help</h3><p>For support, feedback, partnership, or coaching-class customization, send a clear message with your email address.</p></div><div className="card contactInfoCard safe"><ShieldCheck size={24} /><h3>Safe contact only</h3><p>Please do not share passwords, OTPs, API keys, payment details, or private student information in this form.</p></div></aside></div></section>; }
function PrivacyPage() { return <section className="page legalPage"><div className="legalHeader"><span className="landingBadge">Privacy First</span><h1>Privacy Policy</h1><p>Last updated: 23 June 2026</p></div><div className="legalCardStack"><article className="card legalCard"><h3>What we collect</h3><p>Study Blueprint asks for only the basic details needed to personalize your dashboard: name, email, class, exam target, target date, preferred language, study goal, topic progress, PYQ progress, study timer activity and support messages.</p></article><article className="card legalCard"><h3>What we never ask for</h3><p>Please do not share passwords, OTPs, payment card details, home address, private documents or sensitive personal information inside the app or contact form.</p></article><article className="card legalCard"><h3>How we use data</h3><p>Your details are used to show your study dashboard, save your profile, track your target date, record progress and improve the app experience.</p></article></div></section>; }
function ProfilePage({ data, profileDraft, setProfileDraft, editingProfile, setEditingProfile, saveProfile, resetDemoData }) { const [status, setStatus] = useState(''); const fields = [['fullName', 'Full Name', 'text', true], ['mobile', 'Mobile No', 'text', false], ['email', 'Email', 'text', true], ['className', 'Class', 'text', true], ['board', 'Board / State Board', 'text', false], ['language', 'Preferred Language', 'text', false], ['studyGoal', 'Study Goal', 'text', false], ['dailyStudyTime', 'Daily Study Time', 'text', false], ['weakAreas', 'Weak Areas', 'text', false]]; function saveWithStatus() { saveProfile(profileDraft); setStatus('Demo profile saved successfully.'); } return <section className="page profilePageV23"><div className="pageHead"><h1>Profile</h1><p>Manage student details, exam target, profile photo and account settings.</p></div><div className="profileHero profileHeroV23"><div className="profileIdentityBlock"><div className="profilePhotoWrap"><div className="avatar profileAvatarFallback">👨‍🎓</div><label className="photoUploadBtn"><PenLine size={15} /></label></div><div className="profileIdentityText"><h2>{data.profile.fullName || 'Student Profile'}</h2><p>{data.profile.email || 'Email not available'}</p><span className="badge">{data.profile.targetExam || 'Target not set'}</span></div></div><div className="profileFutureCard"><div className="futureGraphic"><span className="rocketShape">🚀</span><i></i><i></i><i></i></div><div><h3>Keep building your future</h3><p>Complete your profile to get a cleaner dashboard, better target tracking and personalized study planning.</p></div></div><div className="targetSummary targetSummaryV23"><span>Target Date</span><b>{data.profile.targetDate ? formatDate(data.profile.targetDate) : 'Date not set'}</b><small>Set target date</small></div></div><div className="card profileTargetCard profileTargetV23"><div><h3>Exam Target</h3><p className="muted">Set the target exam name and date here. The dashboard will use this target for tracking.</p></div><div className="targetForm"><label><span>Target Exam Name</span><input value={profileDraft.targetExam || ''} onChange={(e) => setProfileDraft({ ...profileDraft, targetExam: e.target.value })} /></label><label><span>Target Exam Date</span><input value={profileDraft.targetDate || ''} onChange={(e) => setProfileDraft({ ...profileDraft, targetDate: e.target.value })} type="date" /></label><button className="primary" onClick={saveWithStatus}><Save size={16}/> Save Target</button></div></div><div className="profileGrid profileGridV23"><div className="card wide"><div className="between"><h3>Personal & Academic Details</h3>{editingProfile ? <button onClick={saveWithStatus}><Save size={16}/> Save</button> : <button onClick={() => { setProfileDraft(data.profile); setEditingProfile(true); }}><PenLine size={16}/> Edit</button>}</div><div className="detailsGrid detailsGridV23">{fields.map(([key, label, type, required]) => <label key={key}><span>{label}{!required && <small className="optionalTag">Optional</small>}</span>{editingProfile && key !== 'email' ? <input value={profileDraft[key] || ''} onChange={(e) => setProfileDraft({ ...profileDraft, [key]: e.target.value })} type={type} /> : <b>{data.profile[key] || '-'}</b>}</label>)}</div></div><div className="profileSideStack"><div className="card actions actionsV23"><h3>Account Actions</h3><button className="primary" onClick={() => { setProfileDraft(data.profile); setEditingProfile(true); }}><PenLine size={16}/> Edit Profile</button>{editingProfile && <button onClick={saveWithStatus}><Save size={16}/> Save Changes</button>}<button onClick={() => { resetDemoData(); setStatus('Demo data reset successfully.'); }}><RotateCcw size={16}/> Reset this browser data</button><button className="danger softDanger" onClick={() => setStatus('Demo mode only. Real delete is disabled in preview.')}><Trash2 size={16}/> Demo mode only</button></div>{status && <div className="contactStatusBox">{status}</div>}</div></div></section>; }
